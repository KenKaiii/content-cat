...
export default function VideoEditorNode({
  data,
  selected,
}: NodeProps<Node<VideoEditorNodeData>>) {
  ...
  const operationLabel =
    OPERATION_LABELS[data.operation || "trim"] || "Trim & Cut";

  return (
    <BaseNode
      label={data.label || "Video Editor"}
      selected={selected}
      inputs={[
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            <EditIcon />
            <span className="text-[8px] text-gray-400">{operationLabel}</span>
          </div>
          <span className="text-[8px] text-gray-500">
            {data.transition?.type || "fade"} •{" "}
            {data.transition?.duration || 0.3}s
          </span>
        </div>
      </div>
    </BaseNode>
